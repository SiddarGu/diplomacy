...
export class Page extends React.Component {
    ...
    static __sort_games(games) {
        ...
        games.sort(
            (a, b) => (a.role ? 1 : 0) - (b.role ? 1 : 0) || a.game_id.localeCompare(b.game_id)
        );
        return games;
    }
        ...
        if (game) {
            if (game.client) {
                return game.client
                    ...
                    .catch((error) =>
                        this.error(`Error when deleting game ${gameID}: ${error.toString()}`)
                    );
            } else {
                return this.channel
                ...
                <div className="page container-fluid" id={this.state.contentName}>
                    <div className={"top-msg row"}>
                        ...
                        <div
                            ...
                            className={
                                "col-sm-4 msg error " + (this.state.error ? "with-msg" : "no-msg")
                            }
                            onClick={() => this.error()}
                        >
                            {errorMessage}
